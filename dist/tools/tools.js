...
// 2. Create Quote Tool
export const createQuoteTool = tool(async ({ client_number, project_name, client_name, client_email, }) => {
    const { data, error } = await supabase
        .from(TABLES.QUOTES)
        .insert([
        {
    ...
    name: "create_quote",
    description: "Create a new empty quote for a client. Returns the Quote ID. Requires client name and email.",
    schema: z.object({
        client_number: z
            .string()
...
export const addLineItemTool = tool(async ({ quote_id, parent_line_id, item_catalog_id, description, quantity, unit_price, scope_of_work, }) => {
    ...
    if (!finalCatalogId || !uuidRegex.test(finalCatalogId)) {
        if (finalCatalogId) {
            console.warn(`[addLineItemTool] Invalid or Empty UUID for item_catalog_id: '${finalCatalogId}'. Treating as custom item (null).`);
        }
        finalCatalogId = null;
    }
    console.log(`[addLineItemTool] Final Parent ID to use: ${finalParentId}, Final Catalog ID: ${finalCatalogId}`);
...
export const getQuoteDetailsTool = tool(async ({ quote_id }) => {
    ...
    parents.forEach((p) => console.log(`[getQuoteDetailsTool] Found Parent: ${p.description} (${p.id})`));
...
export const negotiatePriceTool = tool(async ({ line_id, new_unit_price }) => {
...
export const updateLineItemTool = tool(async ({ line_id, quantity, description, unit_price, scope_of_work, }) => {
    ...
    if (scope_of_work !== undefined)
        updates.scope_of_work = scope_of_work;
...
export const deleteLineItemTool = tool(async ({ line_id }) => {
...
// 8. Generate PDF Tool
export const generatePdfTool = tool(async ({ quote_id }) => {
    try {
